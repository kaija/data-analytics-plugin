"""Execute a Gremlin query against an Amazon Neptune graph database.

Loads backend configuration from graph-config.json, creates the
Neptune/Gremlin adapter via the factory, connects, and executes the
given Gremlin query, returning normalized nodes and relationships.
...
def execute_gremlin(query: str, config_path: str = DEFAULT_CONFIG) -> dict:
    """Execute a Gremlin query against the configured Neptune backend.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Execute a Gremlin query against an Amazon Neptune graph database."
    )
    parser.add_argument("query", help="Gremlin query string to execute")
    parser.add_argument(
        "--config",
    ...
    try:
        result = execute_gremlin(args.query, config_path=args.config)
        ...
        print(json.dumps({"error": "execute_gremlin_error", "message": str(exc)}), file=sys.stderr)
        sys.exit(1)
